...
async function query({ user, pwd, host, path, str, insensitive }) {
  return await $`sshpass -p ${pwd} ssh ${user}@${host} 'find ${path} -maxdepth 1 ${insensitive ? '-iname' : '-name'} "*${str}*" | sort -n'`
}
const hash = {}
function processOut(obj, source) {
  const { stdout, stderr } = obj
  if (stderr) console.log(stderr)
  stdout.split(EOL).filter(e => e).forEach(e => {
...
const res1 = await query({
  user: QNAP_USER, pwd: QNAP_PWD, host: QNAP_HOST, path: QNAP_PATH, str, insensitive: inc
})
const res2 = await query({
  user: SYNOLOGY_USER, pwd: SYNOLOGY_PWD, host: SYNOLOGY_HOST, path: SYNOLOGY_PATH, str, insensitive: inc
})
